...
export function Tooltip({ children, content, render }: Props) {
  return (
    <Component.Root>
      <Component.Trigger render={render}>{children}</Component.Trigger>

      <Component.Portal>
        <Component.Positioner sideOffset={4}>
          <Component.Popup className="relative rounded-md bg-black px-1.5 py-0.5 font-bold text-sm text-white dark:bg-white dark:text-black">
