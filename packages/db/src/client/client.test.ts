...
test('buildings carry effects and buffs', async () => {
  ...
  const market = await anno.buildings.get({ id: 3527, lang: 'en' })
  expect(market?.buffs).toEqual([
    { attribute: 'Population', buildingGuid: 3527, value: 1 },
    { attribute: 'Money', buildingGuid: 3527, value: 1 },
  ])
...
test('chains join final building and nodes', async () => {
  const { rows } = await anno.chains.list({ lang: 'en', search: 'Timber' })
  expect(rows.length).toBeGreaterThan(0)
  const chain = await anno.chains.get({ id: 3222, lang: 'en' })
  expect(chain?.building?.guid).toBe(3089)
  expect(chain?.nodes.length).toBeGreaterThan(1)
  expect(chain?.nodes[0]?.parentId).toBeNull()
  expect(await anno.chains.get({ id: 1, lang: 'en' })).toBeNull()
})

test('search ranks name matches, dedupes variants, filters by type and paginates', async () => {
  const bread = await anno.search({ lang: 'en', query: 'bread' })
  ...
  const camps = await anno.search({ lang: 'en', query: 'Infantry Camp' })
...
test('search tolerates typos, word order, case and diacritics', async () => {
  ...
  expect((await anno.search({ lang: 'en', query: 'xyzzyq' })).total).toBe(0)
  const all = await anno.search({ lang: 'en', perPage: 5, query: '' })
